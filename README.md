...
### 💡기능
- Miller Column 드릴다운 — JSON/EJSON key-value 구조 컬럼 단위 탐색
- ObjectId EJSON 타입 자동 판별 + 참조 필드 추적, 연결 문서 점프 (PK 중복 시 후보 선택 모달)
- 컬렉션별 PK/FK 직접 선언으로 참조 등록 <br>
- 인라인 에디터 — 일반 JSON 형식 포함 EJSON 타입(Date/Decimal128/Long)도 그대로 수정, Undo 지원
- 컬럼별 검색 + 정렬 툴바 (DB/컬렉션/문서/필드 전부)
- DB·컬렉션 복제/이름변경/삭제, JSON 파일 드래그앤드롭 import
- 클립보드 복사, .json export
- breadcrumb 경로 네비게이션 + 탐색 컬럼 개수(2~) 조절, 트리 모드도 있음 🪾
- 대용량 문서 애니메이션 비활성화, Load More 패턴
- 프로토타입 ⚠️localstorage 사용이라 최대 약 5MB 사용가능
